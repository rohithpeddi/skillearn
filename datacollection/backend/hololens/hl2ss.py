...
def unpack_packet(data):
	...
	return _packet(timestamp, payload,
	               np.frombuffer(pose, dtype=np.float32).reshape((4, 4)) if (len(pose) == 64) else None)
			...
			if (self._state == 0):
				...
				if (length >= self._size):
					if (self._mode == StreamMode.MODE_1):
						...
						self._pose = np.frombuffer(self._buffer[payload_end:self._size], dtype=np.float32).reshape(
							(4, 4))
					else:
...
class _gatherer:
	def open(self, host, port, chunk_size, mode):
		...
		self._unpacker_bytearray = _unpacker_bytearray()
		self._unpacker_bytearray.reset(mode)
	...
	def get_next_packet_packed(self):
		while True:
			self._unpacker_bytearray.extend(self._client.recv(self._chunk_size))
			if self._unpacker_bytearray.unpack():
				return self._unpacker_bytearray.get()
...
class rx_rm_depth_ahat(_context_manager):
	...
	def open(self):
		self._client = _connect_client_rm_depth_ahat(self.host, self.port, self.chunk, self.mode, self.profile,
		                                             self.bitrate)
...
class rx_pv(_context_manager):
	...
	def open(self):
		self._client = _connect_client_pv(self.host, self.port, self.chunk, self.mode, self.width, self.height,
		                                  self.framerate, self.profile, self.bitrate)
...
def _unpack_rm_depth_ahat_nv12_as_yuv420p(yuv):
	...
	u = yuv[_Mode0Layout_RM_DEPTH_AHAT.BEGIN_AB_U_Y: _Mode0Layout_RM_DEPTH_AHAT.END_AB_U_Y, :].reshape(
		(Parameters_RM_DEPTH_AHAT.HEIGHT, Parameters_RM_DEPTH_AHAT.WIDTH // 4))
	v = yuv[_Mode0Layout_RM_DEPTH_AHAT.BEGIN_AB_V_Y: _Mode0Layout_RM_DEPTH_AHAT.END_AB_V_Y, :].reshape(
		(Parameters_RM_DEPTH_AHAT.HEIGHT, Parameters_RM_DEPTH_AHAT.WIDTH // 4))
...
class _SI_Hand:
	...
	def get_joint_pose(self, joint):
		...
		orientation = np.frombuffer(data[_Mode0Layout_SI_Hand.BEGIN_ORIENTATION: _Mode0Layout_SI_Hand.END_ORIENTATION],
		                            dtype=np.float32)
		position = np.frombuffer(data[_Mode0Layout_SI_Hand.BEGIN_POSITION: _Mode0Layout_SI_Hand.END_POSITION],
		                         dtype=np.float32)
		radius = np.frombuffer(data[_Mode0Layout_SI_Hand.BEGIN_RADIUS: _Mode0Layout_SI_Hand.END_RADIUS],
		                       dtype=np.float32)
		accuracy = np.frombuffer(data[_Mode0Layout_SI_Hand.BEGIN_ACCURACY: _Mode0Layout_SI_Hand.END_ACCURACY],
		                         dtype=np.int32)
...
class unpack_si:
	...
	def get_head_pose(self):
		position = np.frombuffer(self._data[_Mode0Layout_SI.BEGIN_HEAD_POSITION: _Mode0Layout_SI.END_HEAD_POSITION],
		                         dtype=np.float32)
		forward = np.frombuffer(self._data[_Mode0Layout_SI.BEGIN_HEAD_FORWARD: _Mode0Layout_SI.END_HEAD_FORWARD],
		                        dtype=np.float32)
	...
	def get_eye_ray(self):
		origin = np.frombuffer(self._data[_Mode0Layout_SI.BEGIN_EYE_ORIGIN: _Mode0Layout_SI.END_EYE_ORIGIN],
		                       dtype=np.float32)
		direction = np.frombuffer(self._data[_Mode0Layout_SI.BEGIN_EYE_DIRECTION: _Mode0Layout_SI.END_EYE_DIRECTION],
		                          dtype=np.float32)
...
def download_calibration_rm_vlc(host, port):
	data = _download_mode2_data(host, port, _create_configuration_for_rm_mode2(StreamMode.MODE_2),
	                            _Mode2Layout_RM_VLC.FLOAT_COUNT * _SIZEOF.FLOAT)
...
def download_calibration_rm_depth_ahat(host, port):
	data = _download_mode2_data(host, port, _create_configuration_for_rm_mode2(StreamMode.MODE_2),
	                            _Mode2Layout_RM_DEPTH_AHAT.FLOAT_COUNT * _SIZEOF.FLOAT)
	floats = np.frombuffer(data, dtype=np.float32)
	
	uv2x = floats[_Mode2Layout_RM_DEPTH_AHAT.BEGIN_UV2X: _Mode2Layout_RM_DEPTH_AHAT.END_UV2X].reshape(
		Parameters_RM_DEPTH_AHAT.SHAPE)
	uv2y = floats[_Mode2Layout_RM_DEPTH_AHAT.BEGIN_UV2Y: _Mode2Layout_RM_DEPTH_AHAT.END_UV2Y].reshape(
		Parameters_RM_DEPTH_AHAT.SHAPE)
	extrinsics = floats[_Mode2Layout_RM_DEPTH_AHAT.BEGIN_EXTRINSICS: _Mode2Layout_RM_DEPTH_AHAT.END_EXTRINSICS].reshape(
		(4, 4))
	...
	mapx = floats[_Mode2Layout_RM_DEPTH_AHAT.BEGIN_MAPX: _Mode2Layout_RM_DEPTH_AHAT.END_MAPX].reshape(
		Parameters_RM_DEPTH_AHAT.SHAPE)
	mapy = floats[_Mode2Layout_RM_DEPTH_AHAT.BEGIN_MAPY: _Mode2Layout_RM_DEPTH_AHAT.END_MAPY].reshape(
		Parameters_RM_DEPTH_AHAT.SHAPE)
...
def download_calibration_rm_depth_longthrow(host, port):
	data = _download_mode2_data(host, port, _create_configuration_for_rm_mode2(StreamMode.MODE_2),
	                            _Mode2Layout_RM_DEPTH_LONGTHROW.FLOAT_COUNT * _SIZEOF.FLOAT)
	floats = np.frombuffer(data, dtype=np.float32)
	
	uv2x = floats[_Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_UV2X: _Mode2Layout_RM_DEPTH_LONGTHROW.END_UV2X].reshape(
		Parameters_RM_DEPTH_LONGTHROW.SHAPE)
	uv2y = floats[_Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_UV2Y: _Mode2Layout_RM_DEPTH_LONGTHROW.END_UV2Y].reshape(
		Parameters_RM_DEPTH_LONGTHROW.SHAPE)
	extrinsics = floats[
	             _Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_EXTRINSICS: _Mode2Layout_RM_DEPTH_LONGTHROW.END_EXTRINSICS].reshape(
		(4, 4))
	scale = floats[_Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_SCALE: _Mode2Layout_RM_DEPTH_LONGTHROW.END_SCALE]
	mapx = floats[_Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_MAPX: _Mode2Layout_RM_DEPTH_LONGTHROW.END_MAPX].reshape(
		Parameters_RM_DEPTH_LONGTHROW.SHAPE)
	mapy = floats[_Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_MAPY: _Mode2Layout_RM_DEPTH_LONGTHROW.END_MAPY].reshape(
		Parameters_RM_DEPTH_LONGTHROW.SHAPE)
...
def download_calibration_rm_imu(host, port):
	data = _download_mode2_data(host, port, _create_configuration_for_rm_mode2(StreamMode.MODE_2),
	                            _Mode2Layout_RM_IMU.FLOAT_COUNT * _SIZEOF.FLOAT)
	floats = np.frombuffer(data, dtype=np.float32)
...
def download_calibration_pv(host, port, width, height, framerate):
	data = _download_mode2_data(host, port,
	                            _create_configuration_for_pv_mode2(StreamMode.MODE_2, width, height, framerate),
	                            _Mode2Layout_PV.FLOAT_COUNT * _SIZEOF.FLOAT)
	floats = np.frombuffer(data, dtype=np.float32)
	...
	intrinsics = np.array(
		[[-focal_length[0], 0, 0, 0], [0, focal_length[1], 0, 0], [principal_point[0], principal_point[1], 1, 0],
...
class sm_bounding_volume:
	...
	def add_box(self, center, extents):
		self._data.extend(
			struct.pack('<Iffffff', _SM_VolumeType.Box, center[0], center[1], center[2], extents[0], extents[1],
			            extents[2]))
		self._count += 1
	
	def add_frustum(self, near, far, right, left, top, bottom):
		self._data.extend(
			struct.pack('<Iffffffffffffffffffffffff', _SM_VolumeType.Frustum, near[0], near[1], near[2], near[3],
			            far[0], far[1], far[2], far[3], right[0], right[1], right[2], right[3], left[0], left[1],
			            left[2], left[3], top[0], top[1], top[2], top[3], bottom[0], bottom[1], bottom[2], bottom[3]))
		self._count += 1
	
	def add_oriented_box(self, center, extents, orientation):
		self._data.extend(
			struct.pack('<Iffffffffff', _SM_VolumeType.OrientedBox, center[0], center[1], center[2], extents[0],
			            extents[1], extents[2], orientation[0], orientation[1], orientation[2], orientation[3]))
		self._count += 1
...
class sm_mesh_task:
	...
	def add_task(self, id, max_triangles_per_cubic_meter, vertex_position_format, triangle_index_format,
	             vertex_normal_format, include_vertex_normals):
		self._data.extend(
			struct.pack('<16sdIIII', id, max_triangles_per_cubic_meter, vertex_position_format, triangle_index_format,
			            vertex_normal_format, 1 if include_vertex_normals else 0))
		self._count += 1
...
class _sm_mesh:
	def __init__(self, vertex_position_scale, update_time, pose, bounds, vertex_positions, triangle_indices,
	             vertex_normals):
	...
	def unpack(self, vertex_position_format, triangle_index_format, vertex_normal_format):
		...
		self.vertex_positions = np.frombuffer(self.vertex_positions, dtype=_SM_Convert.DirectXPixelFormatToNumPy[
			vertex_position_format]).reshape((-1, 4))
		self.triangle_indices = np.frombuffer(self.triangle_indices, dtype=_SM_Convert.DirectXPixelFormatToNumPy[
			triangle_index_format]).reshape((-1, 3))
		self.vertex_normals = np.frombuffer(self.vertex_normals,
		                                    dtype=_SM_Convert.DirectXPixelFormatToNumPy[vertex_normal_format]).reshape(
			(-1, 4))
...
class su_task:
	def __init__(self, enable_quads, enable_meshes, enable_only_observed, enable_world_mesh, mesh_lod, query_radius,
	             create_mode, kind_flags, get_orientation, get_position, get_location_matrix, get_quad, get_meshes,
	             get_collider_meshes, guid_list):
...
class ipc_su:
	...
	def _download_mesh(self):
		elements_vertices, elements_indices = struct.unpack('<II', self._client.download(2 * _SIZEOF.DWORD,
		                                                                                 ChunkSize.SINGLE_TRANSFER))
	...
	def _download_meshes(self):
		return [self._download_mesh() for _ in
		        range(0, struct.unpack('<I', self._client.download(_SIZEOF.DWORD, ChunkSize.SINGLE_TRANSFER))[0])]
	...
	def _download_item(self, bi, bk, bo, bp, bl, ba, be, bm, download_meshes, download_collider_meshes):
		...
		return _su_item(d[bi:bk], d[bk:bo], d[bo:bp], d[bp:bl], d[bl:ba], d[ba:be], d[be:bm],
		                self._download_meshes() if (download_meshes) else [],
		                self._download_meshes() if (download_collider_meshes) else [])
	
	def query(self, task):
		msg = bytearray()
		msg.extend(struct.pack('<BBBBIfBBBBBBBBI', task.enable_quads, task.enable_meshes, task.enable_only_observed,
		                       task.enable_world_mesh, task.mesh_lod, task.query_radius, task.create_mode,
		                       task.kind_flags, task.get_orientation, task.get_position, task.get_location_matrix,
		                       task.get_quad, task.get_meshes, task.get_collider_meshes, len(task.guid_list)))
		...
		return _su_result(header[he:hp], header[hp:hi], [
			self._download_item(bi, bk, bo, bp, bl, ba, be, bm, task.get_meshes, task.get_collider_meshes) for _ in
			range(0, struct.unpack('<I', header[132:])[0])])
...
class _rs_client:
	def open(self, host, port, max_size):
		...
		self._client = self._loop.run_until_complete(
			websockets.client.connect(_rs_get_stream_url_pull(host, port), max_size=max_size, compression=None))
