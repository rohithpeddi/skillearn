...
class Consumer:
	...
	def _process_stream_data(self, stream_port, stream_data, **kwargs):
		...
		if stream_port == StreamPort.PHOTO_VIDEO:
			...
			# 1. Pose information
			kwargs[PV_POSE_WRITER].write(stream_packet.pose)
			...
			kwargs[SPATIAL_DATA_WRITER].write(stream_packet)
	...
	def _fetch_stream_kwargs(self, stream_port):
		...
		if stream_port == StreamPort.PHOTO_VIDEO:
			...
			kwargs[PV_POSE_WRITER] = FileWriter(stream_file_path, file_extension=".pkl")
			...
			stream_file_path = os.path.join(stream_directory,
			                                f'{self.recording.get_recording_id()}_{stream_name}_pose.pkl')
			kwargs[DEPTH_AHAT_POSE_WRITER] = FileWriter(stream_file_path, file_extension=".pkl")
			...
			kwargs[MICROPHONE_DATA_WRITER] = FileWriter(stream_file_path, file_extension=".pkl")
		elif stream_port == StreamPort.SPATIAL_INPUT:
			# Here we need to save
			# 1. Dump of the spatial data into a pickle file
			stream_file_path = os.path.join(stream_directory, f'{self.recording.get_recording_id()}_{stream_name}.pkl')
			kwargs[SPATIAL_DATA_WRITER] = FileWriter(stream_file_path, file_extension=".pkl")
	...
	def _process_stream(self, stream_port):
		...
		if not done_processing:
			...
			if stream_port == StreamPort.MICROPHONE:
				...
				kwargs[DEPTH_AHAT_POSE_WRITER].close()
			elif stream_port == StreamPort.PHOTO_VIDEO:
				kwargs[PV_POSE_WRITER].close()
			return
