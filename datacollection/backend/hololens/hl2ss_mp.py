...
class _interconnect(mp.Process):
	...
	def _get_nearest(self, sink_din, sink_dout):
		...
		response = (None, None) if (index is None) else (
		self._frame_stamp - self._buffer.length() + 1 + index, buffer[index])
...
class producer:
	...
	def configure_rm_vlc(self, decoded, host, port, chunk, mode, profile, bitrate):
		self.configure(port, hl2ss.rx_decoded_rm_vlc(host, port, chunk, mode, profile, bitrate) if (
			decoded) else hl2ss.rx_rm_vlc(host, port, chunk, mode, profile, bitrate))

	def configure_rm_depth_ahat(self, decoded, host, port, chunk, mode, profile, bitrate):
		self.configure(port, hl2ss.rx_decoded_rm_depth_ahat(host, port, chunk, mode, profile, bitrate) if (
			decoded) else hl2ss.rx_rm_depth_ahat(host, port, chunk, mode, profile, bitrate))

	def configure_rm_depth_longthrow(self, decoded, host, port, chunk, mode, png_filter):
		self.configure(port, hl2ss.rx_decoded_rm_depth_longthrow(host, port, chunk, mode, png_filter) if (
			decoded) else hl2ss.rx_rm_depth_longthrow(host, port, chunk, mode, png_filter))

	def configure_rm_imu(self, host, port, chunk, mode):
		self.configure(port, hl2ss.rx_rm_imu(host, port, chunk, mode))

	def configure_pv(self, decoded, host, port, chunk, mode, width, height, framerate, profile, bitrate,
					 decoded_format):
		self.configure(port, hl2ss.rx_decoded_pv(host, port, chunk, mode, width, height, framerate, profile, bitrate,
												 decoded_format) if (decoded) else hl2ss.rx_pv(host, port, chunk, mode,
																							   width, height, framerate,
																							   profile, bitrate))

	def configure_microphone(self, decoded, host, port, chunk, profile):
		self.configure(port,
					   hl2ss.rx_decoded_microphone(host, port, chunk, profile) if (decoded) else hl2ss.rx_microphone(
						   host, port, chunk, profile))
...
class consumer:
	...
	def create_sink(self, producer, port, manager, semaphore):
		sink_semaphore = None if (semaphore is None) else manager.Semaphore(_interconnect.IPC_SEMAPHORE_VALUE) if (
					semaphore is ...) else self._sink_semaphore[semaphore]
