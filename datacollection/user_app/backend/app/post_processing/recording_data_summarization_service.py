						...
						for pv_data_item in sync_data_item.get_items():
							if pv_data_item.name == const.FRAMES_ZIP:
								self.recording_summary.download_links.HOLOLENS_SYNC_PV_FRAMES_ZIP = pv_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.SYNC} - {const.PV} - {const.FRAMES_ZIP} - {self.recording_summary.download_links.HOLOLENS_SYNC_PV_FRAMES_ZIP}")
								...
								self.recording_summary.download_links.HOLOLENS_SYNC_PV_VIDEO = pv_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.SYNC} - {const.PV} - {const.VIDEO} - {self.recording_summary.download_links.HOLOLENS_SYNC_PV_VIDEO}")
								...
								self.recording_summary.download_links.HOLOLENS_SYNC_PV_POSE_PKL = pv_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.SYNC} - {const.PV} - {const.POSE} - {self.recording_summary.download_links.HOLOLENS_SYNC_PV_POSE_PKL}")
						...
						for spatial_data_item in sync_data_item.get_items():
							if spatial_data_item.name == f"{self.recording_id}_spatial.pkl":
								self.recording_summary.download_links.HOLOLENS_SYNC_SPATIAL_PKL = spatial_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.SYNC} - {const.SPATIAL} - {const.SPATIAL_POSE} - {self.recording_summary.download_links.HOLOLENS_SYNC_SPATIAL_PKL}")
						...
						for imu_data_item in sync_data_item.get_items():
							if imu_data_item.name == f"{self.recording_id}_imu_accelerometer.pkl":
								self.recording_summary.download_links.HOLOLENS_SYNC_IMU_ACCELEROMETER_PKL = imu_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.SYNC} - {const.IMU} - {const.IMU_ACCELEROMETER} - {self.recording_summary.download_links.HOLOLENS_SYNC_IMU_ACCELEROMETER_PKL}")
								...
								self.recording_summary.download_links.HOLOLENS_SYNC_IMU_GYROSCOPE_PKL = imu_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.SYNC} - {const.IMU} - {const.IMU_GYROSCOPE} - {self.recording_summary.download_links.HOLOLENS_SYNC_IMU_GYROSCOPE_PKL}")
								...
								self.recording_summary.download_links.HOLOLENS_SYNC_IMU_MAGNETOMETER_PKL = imu_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.SYNC} - {const.IMU} - {const.IMU_MAGNETOMETER} - {self.recording_summary.download_links.HOLOLENS_SYNC_IMU_MAGNETOMETER_PKL}")
					elif recording_specific_item.name == const.DEPTH_AHAT:
						logger.info(
							f"[{self.recording_id}] Updating download links for {const.SYNC} - {const.DEPTH_AHAT}")
						for depth_ahat_data_item in sync_data_item.get_items():
							if depth_ahat_data_item.name == f"{self.recording_id}_depth_ahat_pose.pkl":
								self.recording_summary.download_links.HOLOLENS_SYNC_DEPTH_POSE_PKL = depth_ahat_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.SYNC} - {const.DEPTH_AHAT} - {const.DEPTH_POSE} - {self.recording_summary.download_links.HOLOLENS_SYNC_DEPTH_POSE_PKL}")
								...
								self.recording_summary.download_links.HOLOLENS_SYNC_DEPTH_AHAT_DEPTH_ZIP = depth_ahat_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.SYNC} - {const.DEPTH_AHAT} - {const.DEPTH_ZIP} - {self.recording_summary.download_links.HOLOLENS_SYNC_DEPTH_AHAT_DEPTH_ZIP}")
								...
								self.recording_summary.download_links.HOLOLENS_SYNC_DEPTH_AHAT_AB_ZIP = depth_ahat_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.SYNC} - {const.DEPTH_AHAT} - {const.AB_ZIP} - {self.recording_summary.download_links.HOLOLENS_SYNC_DEPTH_AHAT_AB_ZIP}")
						...
						for pv_data_item in raw_data_item.get_items():
							if pv_data_item.name == const.FRAMES_ZIP:
								self.recording_summary.download_links.HOLOLENS_RAW_PV_FRAMES_ZIP = pv_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.RAW} - {const.PV} - {const.FRAMES_ZIP} - {self.recording_summary.download_links.HOLOLENS_RAW_PV_FRAMES_ZIP}")
							elif pv_data_item.name == f"{self.recording_id}_pv_pose.pkl":
								self.recording_summary.download_links.HOLOLENS_RAW_PV_POSE_PKL = pv_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.RAW} - {const.PV} - {const.PV_POSE} - {self.recording_summary.download_links.HOLOLENS_RAW_PV_POSE_PKL}")
						...
						for spatial_data_item in raw_data_item.get_items():
							if spatial_data_item.name == f"{self.recording_id}_spatial.pkl":
								self.recording_summary.download_links.HOLOLENS_RAW_SPATIAL_PKL = spatial_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.RAW} - {const.SPATIAL} - {const.SPATIAL_POSE} - {self.recording_summary.download_links.HOLOLENS_RAW_SPATIAL_PKL}")
						...
						for imu_data_item in raw_data_item.get_items():
							if imu_data_item.name == f"{self.recording_id}_imu_accelerometer.pkl":
								self.recording_summary.download_links.HOLOLENS_RAW_IMU_ACCELEROMETER_PKL = imu_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.RAW} - {const.IMU} - {const.IMU_ACCELEROMETER} - {self.recording_summary.download_links.HOLOLENS_RAW_IMU_ACCELEROMETER_PKL}")
							elif imu_data_item.name == f"{self.recording_id}_imu_gyroscope.pkl":
								self.recording_summary.download_links.HOLOLENS_RAW_IMU_GYROSCOPE_PKL = imu_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.RAW} - {const.IMU} - {const.IMU_GYROSCOPE} - {self.recording_summary.download_links.HOLOLENS_RAW_IMU_GYROSCOPE_PKL}")
							elif imu_data_item.name == f"{self.recording_id}_imu_magnetometer.pkl":
								self.recording_summary.download_links.HOLOLENS_RAW_IMU_MAGNETOMETER_PKL = imu_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.RAW} - {const.IMU} - {const.IMU_MAGNETOMETER} - {self.recording_summary.download_links.HOLOLENS_RAW_IMU_MAGNETOMETER_PKL}")
					elif raw_data_item.name == const.DEPTH_AHAT:
						logger.info(
							f"[{self.recording_id}] Updating download links for {const.RAW} - {const.DEPTH_AHAT}")
						for depth_ahat_data_item in raw_data_item.get_items():
							if depth_ahat_data_item.name == f"{self.recording_id}_depth_ahat_pose.pkl":
								self.recording_summary.download_links.HOLOLENS_RAW_DEPTH_POSE_PKL = depth_ahat_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.RAW} - {const.DEPTH_AHAT} - {const.DEPTH_POSE} - {self.recording_summary.download_links.HOLOLENS_RAW_DEPTH_POSE_PKL}")
							elif recording_specific_item.name == const.DEPTH_ZIP:
								self.recording_summary.download_links.HOLOLENS_RAW_DEPTH_AHAT_DEPTH_ZIP = depth_ahat_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.RAW} - {const.DEPTH_AHAT} - {const.DEPTH_ZIP} - {self.recording_summary.download_links.HOLOLENS_RAW_DEPTH_AHAT_DEPTH_ZIP}")
							elif recording_specific_item.name == const.AB_ZIP:
								self.recording_summary.download_links.HOLOLENS_RAW_DEPTH_AHAT_AB_ZIP = depth_ahat_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.RAW} - {const.DEPTH_AHAT} - {const.AB_ZIP} - {self.recording_summary.download_links.HOLOLENS_RAW_DEPTH_AHAT_AB_ZIP}")
						...
						for mc_data_item in raw_data_item.get_items():
							if mc_data_item.name == f"{self.recording_id}_mc.pkl":
								self.recording_summary.download_links.HOLOLENS_RAW_MC_PKL = mc_data_item.get_shared_link_download_url(
									access='open')
								logger.info(
									f"[{self.recording_id}] Download link for {const.RAW} - {const.MC} - {const.MC_PKL} - {self.recording_summary.download_links.HOLOLENS_RAW_MC_PKL}")
