...
class SynchronizationServiceV2:
	...
	def create_sync_stream_pkl_data(self, stream_pkl_file_path, sync_stream_output_directory, base_ts_to_stream_ts):
		if files_have_same_size(stream_pkl_file_path, sync_stream_output_directory):
			logger.info(f"[{self.recording_id}] Skipping pkl file {stream_pkl_file_path}")
			return
	...
	def create_sync_stream_frames(
		...
		if not directories_have_same_file_count(raw_stream_directory, sync_stream_output_directory):
			...
			logger.info(
				f"[{self.recording_id}] Skipping copying frames from {raw_stream_directory} to {sync_stream_output_directory}")
		return
	...
	def sync_streams(self):
		...
		if not directories_have_same_file_count(raw_base_stream_frames_dir, sync_base_stream_frames_dir):
			...
			logger.info(f"[{self.recording_id}] Base stream frames already exist in the sync output folder")
