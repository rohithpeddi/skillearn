...
def process_files_in_folder(source_folder):
	...
	for file_path in all_files:
		...
		if len(filename_parts) > 1:
			...
			new_folder = os.path.join(source_folder, recipe_name)
	...
	with open("execution_scripts.txt", "w") as execution_file:
		for file in os.listdir(path):
			...
			if os.path.isfile(os.path.join(path, file)):
				script += f"CUDA_VISIBLE_DEVICES=0 python -m RepLearn.TCC.main --cfg configs/{file}"
...
if __name__ == '__main__':
	...
	create_directories('/home/rohith/data/ptg')
